...
def train_city_redisual_model_trend(args, city_index):
    ...
    base_model.load_state_dict((torch.load(os.path.join(cfg.LOG.home_log_dir, '12-15-21_GLUPoint4layerAddTrend_Train/model/epoch_199.model'))))
    ...
    print(city_index, c, city_district_num)


    train_city_residual_glu_model(base_model, c_in, train_loader, val_loader, c, city_district_num, city_district_index_list)
    ...
    preds = torch.cat([(base_preds.cpu().double()-mean_flow)/std_flow, input_flow_x[:,3:,:,23:24], ], dim=1)# 1 x H x 98 x 1

    for i in range(1, cfg.DATASET.middle_result_days):
        input_flow_x = torch.cat([input_flow_x[:,:,:,1:], preds], dim=-1)
    ...
    preds_df.to_csv(os.path.join(cfg.LOG.experiment_result_dir, 'trend_city_resudial_prediction_{}_{}_{}.csv'.format(cfg.LOG.timestamp, p, c)), index=False, header = False)


def train_city_redisual_models_trend(args):
    seed = cfg.TRAIN.seed#87#14
    torch.manual_seed(seed)
    torch.cuda.manual_seed_all(seed)
    np.random.seed(seed)
    ...
    city_code_list = dataset.city_code_list
    city_district_map, city_district_index_map = dataset.city_district_map()
    date_list = dataset.test_date_generate()
    ...
    base_model.load_state_dict((torch.load(os.path.join(cfg.LOG.home_log_dir, '12-13-16_GLUPoint1214_Train/model/epoch_199.model'))))
    ...
    for i,c in enumerate(city_code_list):
        ...
        city_residual_preds = model(flow_x[:,:,city_district_index_list,:], transition_x, constant_attention=True)
        base_preds = base_model(flow_x[:,:3,:,:], transition_x, constant_attention=True)
        base_preds[:,:,city_district_index_list,:] = base_preds[:,:,city_district_index_list,:] + city_residual_preds
        preds = torch.cat([(base_preds.cpu().double()-mean_flow)/std_flow, input_flow_x[:,3:,:,23:24], ], dim=1)# 1 x H x 98 x 1
        ...
        for i in range(1, cfg.DATASET.result_days):
            ...
            print(base_preds.shape, input_flow_x.shape)
            preds = torch.cat([(base_preds.cpu().double()-mean_flow)/std_flow, input_flow_x[:,3:,:,23:24], ], dim=1)# 1 x H x 98 x 1

        preds = torch.cat([(input_flow_x[:,:3,:,-1*(cfg.DATASET.result_days-1):] * std_flow) + mean_flow, base_preds.cpu().double()], dim=-1)# 1 x 3 x 98 X 15
        np_preds = preds.cpu().data.numpy()
        np_preds = np.transpose(np_preds, axes=[0,2,3,1])# 1 x 3 x 98 x 15
        print(np_preds.shape, preds.shape)
        ...
        for index,district_code in enumerate(city_district_code_list):
            ...
            tmp_df['district_code'] = district_code
            for i in range(3):
                district_element_preds = pd.Series(np_preds[0,district_index,:,i])
                if cfg.DATASET.flow_transform:
                    district_element_preds = np.exp(district_element_preds) - 1
                tmp_df = pd.concat([tmp_df, district_element_preds], axis=1)
...
def train_city_residual_glu_model(base_model, c_in, train_loader, val_loader, city_code, city_district_num, city_district_code_list):
    seed = cfg.TRAIN.seed#87#14
    torch.manual_seed(seed)
    torch.cuda.manual_seed_all(seed)
    np.random.seed(seed)
