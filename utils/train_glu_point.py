...
from model.glu import GLUModel, MediumGLUModel, ShortGLUModel, LongGLUModel, PeriodGLUModel, LSGLUModel
...
def train_GLUPoint(args, date_index=None):
    ...
    if cfg.DATASET.T%30==0 or cfg.DATASET.T == 28:
        if cfg.TRAIN.period:
            ...
            # model.load_state_dict((torch.load(os.path.join(cfg.LOG.home_log_dir, '12-16-23_GLUPoint_Pre-Train/model/epoch_199.model'))))
    ...
    elif cfg.DATASET.T == 40:
        print('--------------------------------LSGLU 40 Model-------------------------------')
        model = LSGLUModel(c_in, input_days=cfg.DATASET.T).cuda()
    ...
    for p in range(cfg.TRAIN.epochs):
        ...
        if (p+1) % cfg.TRAIN.test_time == 0:
            ...
            if date_index == None:
                ...
                # N x 3 x 1 x 1
                # preds = preds.permute(2,1,0,3)
                for i in range(1, cfg.DATASET.middle_result_days):
                    input_flow_x = torch.cat([input_flow_x[:,:3,:,1:], (preds.cpu().double()-mean_flow)/std_flow], dim=-1)
                    # input_flow_x = torch.cat([input_flow_x, days_weighted], dim=1)
                    input_flow_x = input_flow_x.data
                    flow_x.resize_(input_flow_x.size()).copy_(input_flow_x)
                    preds = model(flow_x, transition_x, constant_attention=True)
                    # preds = preds.permute(2,1,0,3)# 204 x 3 x 1 x 1
                ...
                # generate result
                final_preds = torch.cat([(input_flow_x[:,:3,:,-1*(cfg.DATASET.final_result_days-1):] * std_flow) + mean_flow, preds.cpu().double()], dim=-1)
