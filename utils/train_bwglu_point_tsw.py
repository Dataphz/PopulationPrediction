...
def train_BWGLUPointTrendSeansonWeight(args, date_index=None):
    ...
    train_transition, train_flow_x, train_flow_y, val_transition, val_flow_x, val_flow_y, test_transition, middle_test_flow_x = dataset.backward_generate()
    ...
    for p in range(cfg.TRAIN.epochs):
        ...
        if (p+1) % cfg.TRAIN.test_time == 0:
            ...
            if date_index == None:
                ...
                # print(preds, input_flow_x.shape)
                preds_input = torch.cat([(preds.cpu().double()-mean_flow)/std_flow, input_flow_x[:,3:9,:,cfg.DATASET.T-7:cfg.DATASET.T-6], ], dim=1)# 1 x H x N x 1

                for i in range(1, cfg.DATASET.middle_result_days):
                    input_flow_x = torch.cat([input_flow_x[:,:9,:,1:], preds_input], dim=-1)
                    input_flow_x = torch.cat([input_flow_x, days_weighted], dim=1) # bs x 7 x 1 x T

                    input_flow_x = input_flow_x.data
                    flow_x.resize_(input_flow_x.size()).copy_(input_flow_x)
                    preds = model(flow_x, transition_x, constant_attention=True)
                    preds_input = torch.cat([(preds.cpu().double()-mean_flow)/std_flow, input_flow_x[:,3:9,:,cfg.DATASET.T-7:cfg.DATASET.T-6], ], dim=1)
                ...
                backward_middle_generate_result(np_middle_preds[:,:,::-1,:], middle_date_list, p, dataset.district_code_list, dataset.district_city_map())
